...
int app(const std::vector<std::string>& args) {
    if (args.size() < 2 || args[1] == "--help") {
        std::cout << "Luna Audio AI - local RNNoise prototype\n"
          "  luna-cli devices\n"
    ...
    if (command == "wav") { report(luna::processWav(std::filesystem::u8path(args[2]), std::filesystem::u8path(args[3]), strength, enabled)); return 0; }
